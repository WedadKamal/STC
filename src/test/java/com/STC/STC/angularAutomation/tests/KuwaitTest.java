...
public class KuwaitTest extends BaseTest{


    ArrayList KuwaitTestData = new ArrayList();
    String actualResults;
    @BeforeClass(alwaysRun = true)
    public void navigateToKuwaitPage(){
        ...
        navigatedToPageSuccessfully = STCHomePage.selectKuwaitCountry();
        Assert.assertEquals(navigatedToPageSuccessfully, GeneralConstants.SUCCESS, "Could not navigate to Kuwait Page successfully");
    }

    @Test(description = "Validate Kuwait functionalities",priority = 0  ,dataProvider = "KuwaitDP" ,enabled = true)
    public void verifyKuwaitFunctionality(STCHomePageDM STCHomePageDM) {
    ...
    @DataProvider(name = "KuwaitDP")
    public Object[][] provideKuwaitTD() {
        Object[][] STCHomePageDP = new Object[KuwaitTestData.size()][1];
        for (int i = 0; i < KuwaitTestData.size(); i++)
            STCHomePageDP[i][0] = KuwaitTestData.get(i);

        return STCHomePageDP;
    }

    @BeforeClass
    public void KuwaitTD() {

        ArrayList<ArrayList<Object>> resultArray = provideTestData("KuwaitTestData");
        ...
        for (ArrayList<Object> objects : resultArray) {
            ...
            KuwaitTestData.add(STCHomePageDM);
