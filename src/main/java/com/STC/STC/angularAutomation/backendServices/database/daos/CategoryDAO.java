...
import com.STC.STC.angularAutomation.constants.database.CategoryDBTable;
...
public class CategoryDAO extends MainDAO{
    ...
    public CategoryDM getCategoryDetails(Connection dbConn, String CategoryEnName) throws SQLException {
        ...
        query.append(        "select MERCHANT_ITEM_CATEGORY.ID ,MERCHANT_ITEM_CATEGORY.NAME_PRIMARY_LANG, MERCHANT_ITEM_CATEGORY.NAME_SECONDARY_LANG,\n" +
                        ...
                        " MERCHANT_ITEM_CATEGORY.STATUS_ID from MERCHANT_ITEM_CATEGORY where MERCHANT_ID = (select ECOMMERCE_MERCHANTS.ID from ECOMMERCE_MERCHANTS where ECOMMERCE_MERCHANTS.EMAIL='"+validUserMail+"')  AND MERCHANT_ITEM_CATEGORY.NAME_PRIMARY_LANG = '"+CategoryEnName+"'");
        // Execute query
    ...
    public CategoryDM getAllCategoryDetails(Connection dbConn) throws SQLException {
        ...
        StringBuilder query = new StringBuilder();
        query.append("      select MERCHANT_ITEM_CATEGORY.ID ,MERCHANT_ITEM_CATEGORY.NAME_PRIMARY_LANG, MERCHANT_ITEM_CATEGORY.NAME_SECONDARY_LANG,\n" +
                "  MERCHANT_ITEM_CATEGORY.CATEGORY_IMAGE_URL from MERCHANT_ITEM_CATEGORY where MERCHANT_ITEM_CATEGORY.MERCHANT_ID = (select ECOMMERCE_MERCHANTS.ID from ECOMMERCE_MERCHANTS where ECOMMERCE_MERCHANTS.EMAIL='"+validUserMail+"') "
                        + "AND MERCHANT_ITEM_CATEGORY.STATUS_ID=1");
...
/*            CategoryDM = new CategoryDM();
        ...
        CategoryDM.setArCategoryName(CatArName.get(0));
        CategoryDM.setCategoryimage(Image.get(0));
    ...
    public CategoryDM getCategoryStatus(Connection dbConn, String CategoryEnName) throws SQLException {
        ...
        StringBuilder query = new StringBuilder();
        query.append(
        "select MERCHANT_ITEM_CATEGORY.STATUS_ID from MERCHANT_ITEM_CATEGORY where \n" +
                "MERCHANT_ITEM_CATEGORY.MERCHANT_ID = (select ECOMMERCE_MERCHANTS.ID from ECOMMERCE_MERCHANTS where" +
                " ECOMMERCE_MERCHANTS.EMAIL='"+validUserMail+"') "+
        ...
        {
            CategoryDM = new CategoryDM();
            CategoryDM.setSTATUS_ID(CategoryRS.getString(CategoryDBTable.STATUS_ID)==null?"":CategoryRS.getString(CategoryDBTable.STATUS_ID));
    ...
    public CategoryDM getCategoryEventsCount(Connection dbConn,String CategoryEnName) throws SQLException {
     ...
     //   CategoryDM CategoryDM = null;
        ...
        if (CategoryDM==null) {
            CategoryDM.setCategoryEventsCount(0);
        }
        return CategoryDM;
    }
